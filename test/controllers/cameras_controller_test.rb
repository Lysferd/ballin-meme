...
class CamerasControllerTest < ActionController::TestCase
  ...
  test "should create camera" do
    assert_difference('Camera.count') do
      post :create, camera: { ipv4: @camera.ipv4, label: @camera.label, password: @camera.password, username: @camera.username }
    end
  ...
  test "should update camera" do
    patch :update, id: @camera, camera: { ipv4: @camera.ipv4, label: @camera.label, password: @camera.password, username: @camera.username }
    assert_redirected_to camera_path(assigns(:camera))
  end
